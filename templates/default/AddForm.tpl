            ...
            <fieldset class="galette_form" id="general">
                ...
                {else}
                    <table width=100%>
                        <thead>
                            <tr>
                                <th class="left"><a href="{path_for name='objectslend_objects'}">
                                        {_T string='Name' domain='ski'}</a></th>
                                ...
                                {if $mbr['parent_id'] == $parent_id}
                                    ...
                                    <tr>
                                        ...
                                        {foreach $categories as $cat}
                                            ...
                                            {$rentobj='-'}
                                            {$used=0}
                                            {if array_key_exists($form_id,$form_rents)}
                                                {if array_key_exists($id_adh,$form_rents[$form_id])}
                                                    {if array_key_exists($category_id,$form_rents[$form_id][$id_adh])}
                                                        {if array_key_exists('object_id', $form_rents[$form_id][$id_adh][$category_id])}
                                                            {$b=$form_rents[$form_id][$id_adh][$category_id]['object_id']}
                                                            {$state=$objects[$b]['state']}
                                                            {$serial=$objects[$b]['serial_number']}
                                                            {$rentobj=$objects[$b]['name']}
                                                            {if array_key_exists('used', $objects[$b])}
                                                                {$used=$objects[$b]['used']}
                                                            {/if}
                                                            {if $state=="used" || $state == 'libre'}
                                                                {if $used < 2} {$rentobj=$objects[$b]['name1']} {/if} {/if} {/if} {/if} {/if} {/if} <td class="center">
                                                                    {if $state=="used" || $state == 'libre'}
                                                                        {if $used < 2} <font color=green>
                                                                            {else}
                                                                                <font color=red>
                                                                                {/if}
                                                                            {elseif $state=="during"}
                                                                                ...
                                                                                {else}
                                                                                    ...
                                                                                    {if $form_status eq 'Open'}
                                                                                        &emsp;
                                                                                        <select name="object" id="object" style="width: 80% ">
                                                                                            {foreach from=$objects item=obj}
                                                                                                {$object_category_id=$obj['category_id']} {$object_id=$obj['object_id']}
                                                                                                {$object_name=$obj['name']}
                                                                                                {if $category_id eq $object_category_id}
                                                                                                    {assign var="mbr1" value={"object="}}
                                                                                                    {assign var="mbr2" value="{$mbr1}{":form_id="}{$form_id}"}
                                                                                                    {assign var="mbr3" value="{$mbr2}{":id_adh="}{$id_adh}"}
                                                                                                    {assign var="mbr4" value="{$mbr3}{":sname="}{$sname}"}
                                                                                                    {assign var="mbr5" value="{$mbr4}{":category_id="}{$category_id}"}
                                                                                                    {assign var="mbr6" value="{$mbr5}{":object_id="}{$object_id}"}
                                                                                                    {assign var="mbr7" value="{$mbr6}{":object_name="}{$obj["name1"]}"}
                                                                                                    {assign var="mbr8" value="{$mbr7}{":date_begin="}{$date_begin}"}
                                                                                                    {assign var="mbr9" value="{$mbr8}{":date_forecast="}{$date_forecast}"}
                                                                                                    {assign var="mbr10" value="{$mbr9}{":date_end="}{$date_end}"}
                                                                                                    {assign var="mbr11" value="{$mbr10}{":period="}{$period}"}
                                                                                                    {assign var="mbr12" value="{$mbr11}{":duration="}{$duration}"}
                                                                                                    <option value="{$mbr12}">
                                                                                                        {$obj['name']}
                                                                                                    </option>
                                                                                                {/if}
