{if $login->isLogged()}
    ...
    <ul>
        <li {if $cur_route eq 'ski_dashboard' } class="selected" {/if}>
        ...
        <li {if $cur_route eq 'ski_family' } class="selected" {/if}>
            <a href="{path_for name=ski_family data=[option=>add] domain='ski'}" title="{_T string='New Family' domain='ski'}">
                {_T string='New Family' domain='ski'}
            </a>
        </li>
    </ul>
{/if}